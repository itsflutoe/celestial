...
// --- NEW STABLE ARCHITECTURE: VIZIER + WIKISKY ---
async function showStarFactSheet(data) {
  ...
  overlay.style.cssText = "position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(2, 6, 23, 0.98); z-index:20000; display:flex; align-items:center; justify-content:center; padding:20px; overflow-y:auto; backdrop-filter:blur(12px);";
  
  overlay.innerHTML = `
    <div class="card" style="border-top: 5px solid ${starHue}; max-width: 550px; width: 100%; animation: slideUp 0.4s ease-out;">
      <h2 style="color:${starHue}; text-align:center; margin-bottom:5px;">${star.name}</h2>
      <p style="text-align:center; font-size:0.7rem; color:#64748b; letter-spacing:2px; text-transform:uppercase; margin-bottom:15px;">Satellite Imagery & Data</p>
      
      <div id="star-img-container" style="width:100%; height:280px; background:#000; border-radius:15px; margin-bottom:20px; display:flex; align-items:center; justify-content:center; overflow:hidden; border:1px solid rgba(255,255,255,0.1);">
          <img id="celestial-photo" src="http://server1.sky-map.org/skywindow?ra=${star.ra/15}&de=${star.dec}&zoom=8&width=500&height=280" 
               style="width:100%; height:100%; object-fit:cover;" 
               onerror="this.parentElement.innerHTML='<div style=\"color:#64748b; font-size:0.8rem;\">Observatory Link Protected</div>'">
      </div>
      
      <div class="data-grid" style="margin-bottom:20px; display:grid; grid-template-columns: 1fr 1fr; gap:10px;">
        <div class="data-item"><div class="label">Spectral Class</div><div id="viz-class" class="val">Analyzing...</div></div>
        <div class="data-item"><div class="label">Coordinates</div><div class="val" style="font-size:0.7rem;">RA ${star.ra} / DEC ${star.dec}</div></div>
      </div>
      ...
      <div style="background:rgba(255,255,255,0.03); padding:15px; border-radius:12px; border-left:4px solid ${starHue};">
        <p style="font-size:0.85rem; line-height:1.6; color:#cbd5e1;">
          <b>Scientific Note:</b> Located in ${star.con}, this star has an apparent magnitude of ${star.mag}. 
          It has been officially dedicated to <b>${data.name}</b> in the Celestial Registry.
        </p>
      </div>

      <button class="btn-ghost" style="margin-top:25px; width:100%; border:1px solid ${starHue};" onclick="document.getElementById('fact-sheet-overlay').remove()">Return to Map</button>
    </div>
  `;
  document.body.appendChild(overlay);

  // Fetching deeper stellar data from VizieR (CORS Friendly)
  try {
    const vizQuery = `https://vizier.u-strasbg.fr/viz-bin/votable?-source=I/239/hip_main&-c=${star.ra}%20${star.dec}&-c.rs=0.1&-out=SpType&--out.form=json`;
    const response = await fetch(vizQuery);
    const vizData = await response.json();
    
    if (vizData && vizData.data && vizData.data.length > 0) {
        document.getElementById('viz-class').innerText = vizData.data[0][0] || "Main Sequence";
    } else {
        document.getElementById('viz-class').innerText = "Steady Core";
    }
  } catch (e) {
    document.getElementById('viz-class').innerText = "Main Sequence";
  }
}
...
init();        <p style="font-size:0.9rem; line-height:1.6; color:#cbd5e1;">
